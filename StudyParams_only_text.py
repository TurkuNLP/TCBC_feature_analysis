...
def manualStudy(params, SNIPPET_LENS, keylists, i, k, cache_dir, overwrite: bool=True):
    ...
    if overwrite or not os.path.exists(filename):
        ...
        train_dss = cmf.combineSnippedBooksToDS(train_keys, SNIPPET_LENS[k], cache_file, BASE_BEG)
        eval_dss = cmf.combineSnippedBooksToDS(eval_keys, SNIPPET_LENS[k], cache_file, BASE_BEG)
...
def testParamResults(permutations: int, keylists: list):
    ...
    for i in range(permutations):
        ...
        for k in range(len(SNIPPET_LENS)):
            pool.apply_async(manualStudy, [CHOSEN_PARAMS, SNIPPET_LENS, keylists, i, k, cache_dir], callback=update)
    #print("All running!")
    pool.close()
    #print("Pool closed!")
    pool.join()
