...
train_ds = train_ds.map(mapLabels, batched=True, batch_size=32)
train_ds = train_ds.map(mapConlluData2RawText, batched=True, batch_size=16)
#Evaluation dataset
eval_ds = base_dataset.filter(lambda x: x['book_id'] in eval_keys).shuffle()
eval_ds = eval_ds.map(mapLabels, batched=True, batch_size=32)
eval_ds = eval_ds.map(mapConlluData2RawText, batched=True, batch_size=16)
#Test dataset
test_ds = base_dataset.filter(lambda x: x['book_id'] in test_keys).shuffle()
test_ds = test_ds.map(mapLabels, batched=True, batch_size=32)
test_ds = test_ds.map(mapConlluData2RawText, batched=True, batch_size=16)
