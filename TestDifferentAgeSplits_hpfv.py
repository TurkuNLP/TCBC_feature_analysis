...
def initDatasets():
    ...
    os.mkdir(cache_dir)
    train_ds = cmf.combineSnippedBooksToDS(train_keys, '50', cache_dir, cache_file=cache_dir+"0_50_train.jsonl", folder=BASE_BEG, inc_hpfv=True)
    eval_ds = cmf.combineSnippedBooksToDS(eval_keys, '50', cache_dir, cache_file=cache_dir+"0_50_eval.jsonl", folder=BASE_BEG, inc_hpfv=True)
    test_ds = cmf.combineSnippedBooksToDS(test_keys, '50', cache_dir, cache_file=cache_dir+"0_50_test.jsonl", folder=BASE_BEG, inc_hpfv=True)
...
def evaluateGroups(train_ds: Dataset, eval_ds: Dataset, test_ds: Dataset, intervals):
    ...
    vectorized_train = train_ds['hp_fv']
    vectorized_eval = eval_ds['hp_fv']
    vectorized_test = test_ds['hp_fv']
    ...
    #Write JSON-file
    with open("TestResults/DifferentAgeGroups_hpfv/"+id+".json", 'w') as f:
